 ...
 *			Modified by Ventsislav Zhechev on 25 Jan 2013
 *			Improved some of the debug output.
 *
 *			Modified by Ventsislav Zhechev on 15 Mar 2011
 *
 */
		...
		while(getline(inFile, line)){
			...
			if(currKey.empty()){
				...
				if(d == InvalidOffT) { 
					...
					TRACE_ERR("ERROR: source phrase already inserted (A)!\nline(" << lnc << "): '" << line << "' at offset " << d << "\n");
					return false;
				}
			}
			if(currKey != key){
#ifdef TRACE_ENABLE
				TRACE_ERR("Finished processing key: ");
				for (IPhrase::const_iterator it = currKey.begin(); it != currKey.end(); ++it) {
					TRACE_ERR(" " << *it << ",");
				}
				TRACE_ERR(std::endl);
				TRACE_ERR("New key: ");
				for (IPhrase::const_iterator it = key.begin(); it != key.end(); ++it) {
					TRACE_ERR(" " << *it << ",");
				}
				TRACE_ERR(std::endl);
#endif
...
#ifdef TRACE_ENABLE
					TRACE_ERR("ERROR: source phrase already inserted (B)!\nline(" << lnc << "): '" << line << "' at offset " << d << std::endl << "KEY:");
					for (IPhrase::const_iterator it = key.begin(); it != key.end(); ++it) {
						TRACE_ERR(" " << *it << ",");
					}
					TRACE_ERR(std::endl);
