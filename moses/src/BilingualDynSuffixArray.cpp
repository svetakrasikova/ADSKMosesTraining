 ...
 *			Last modified by Ventsislav Zhechev on 23 Mar 2011
 *
 */
...
vector<unsigned> BilingualDynSuffixArray::SampleSelection(vector<unsigned> sample) const {
	vector<unsigned>::size_type size = sample.size();
	//if(size < m_maxSampleSize) return sample;
	vector<unsigned> subSample;
	vector<unsigned>::size_type jump = size / m_maxSampleSize;
...
void BilingualDynSuffixArray::addSntPair(string& source, string& target, string& alignment) {
  ...
  istringstream sss(source), sst(target), ssa(alignment);
  string word;
  const vector<wordID_t>::size_type oldSrcCrpSize = m_srcCorpus->size(), oldTrgCrpSize = m_trgCorpus->size();
