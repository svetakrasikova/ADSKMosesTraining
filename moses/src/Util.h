// $Id: Util.h 3078 2010-04-08 17:16:10Z hieuhoang1972 $
...
© 2013 Autodesk Development Sàrl
 Last modified by Ventsislav Zhechev on 25 Jan 2013
 Added forced output flushing to the error output.
 ...
 * */
#ifdef TRACE_ENABLE
#define TRACE_ERR(str) do { std::cerr << str << std::flush; } while (false)
#else
...
{
	std::set<const void*> seenPointers;
	for (typename COLL::iterator iter = coll.begin() ; iter != coll.end() ; ++iter)
	{
		if (*iter != NULL && seenPointers.find(*iter) == seenPointers.end()) {
//			std::cerr << "™™™ Deleting " << *iter << std::endl;
			delete (*iter);
			seenPointers.insert(*iter);
		}
	}
	coll.clear();
	seenPointers.clear();
